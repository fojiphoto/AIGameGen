...
class Button(Widget):
    ...
    def draw(self, surf):
        ...
        if self.icon is not None:
            ic = assets.power_icon(self.icon, int(r.h * 0.46))
            tinted = ic.copy()
            tinted.fill((*accent, 255), special_flags=pygame.BLEND_RGBA_MULT)
            surf.blit(tinted, tinted.get_rect(center=(r.x + 34, r.centery)))
