 ...
 * Build EMBERWAKE.
 *
 *   node Emberwake/build.mjs           game + page  ->  build/web/
 *   node Emberwake/build.mjs --test    core as ESM  ->  build/test/  (for node --test)
 *
 * Same shape as IVORY GAMBIT's, for the same reasons: esbuild for speed, an unminified ESM
 * bundle of the DOM-free half so the suite can run in Node, and a separate `tsc --noEmit` pass
