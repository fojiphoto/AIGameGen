#!/usr/bin/env node
/**
 * Publish FEATHER & FETCH.
 *
 *   npm run publish:featherfetch
 *
 ...
 * run is how a duck nobody can hit reaches a live page.
 *
 * Flags:  --skip-tests   build without the fairness sweep
...
console.log('\n\x1b[1mFEATHER & FETCH — publish\x1b[0m');

step('1/4  type check');
try {
  await access(join(ROOT, 'node_modules', 'typescript', 'lib', 'tsc.js'));
  run(node, [join('node_modules', 'typescript', 'lib', 'tsc.js'),
    '-p', join('FeatherFetch', 'tsconfig.json')], 'type check');
...
if (skipTests) {
  ...
  run(node, [join('FeatherFetch', 'build.mjs'), '--test'], 'test bundle');
  run(node, ['--test', 'FeatherFetch/test/*.test.mjs'], 'tests');
}

step('3/4  build');
run(node, [join('FeatherFetch', 'build.mjs')], 'build');
...
console.log('\n\x1b[32m✓ FEATHER & FETCH published\x1b[0m');
console.log('  live  https://fojiphoto.github.io/AIGameGen/play/feather-and-fetch/\n');
