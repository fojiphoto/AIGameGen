 ...
 *   3. Rebuild the arcade site, which copies that folder into docs/play/block-bloom/ and rewrites
...
// 1280x720 canvas and writes an aspect ratio of 1.77 into the page, so without these the game
// draws its 720x1280 surface into a landscape frame and the player gets a narrow strip between two
// black bars. Telling the template the real shape is the whole fix.
...
/**
 * Correct the aspect ratio pygbag writes into its own page.
 *
 ...
 *
 * Patched here rather than by shipping a custom template, because a fork of pygbag's page would
 * have to be re-synced every time pygbag updates, and this is a one-line disagreement.
 */
async function fixAspect() {
  const page = join(GAME, 'build', 'web', 'index.html');
  const src = await readFile(page, 'utf8');
  const want = (720 / 1280).toFixed(4);
  const patched = src.replace(/fb_ar\s*:\s*[\d.]+/, `fb_ar   :  ${want}`);
  if (patched === src) {
    console.log('\n\x1b[33mfb_ar not found in the generated page — pygbag\'s template may have '
                + 'changed; check the canvas is not stretched\x1b[0m');
    return;
  }
  await writeFile(page, patched, 'utf8');
  console.log(`\n\x1b[1mfix the canvas aspect\x1b[0m \x1b[2mfb_ar 1.77 -> ${want} (portrait)\x1b[0m`);
}
...
console.log(`    \x1b[2mgit add -A && git commit -m "Update BLOCK BLOOM" && git push\x1b[0m`);
