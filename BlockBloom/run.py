...
Why a launcher rather than "pip install -r requirements.txt && python -m blockbloom": because the
...
def main() -> int:
    if sys.version_info < MIN_PYTHON:
        print(f"BLOCK BLOOM needs Python {MIN_PYTHON[0]}.{MIN_PYTHON[1]} or newer; "
              f"this is {sys.version.split()[0]}.")
        return 1

    missing = _missing()
    if missing:
        print("BLOCK BLOOM — first run, fetching what it needs.")
    ...
    from blockbloom.__main__ import main as game_main
