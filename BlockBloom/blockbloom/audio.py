...
def _build_bank_py() -> None:
    ...
    pl_noise = _py_noise(0.13, 23, 0.5)
    pl_env = _py_env(len(pl_noise), 0.001, 0.03, 0.0, 0.03)
    _bank["place"] = _py_make([
        _py_voice(300.0, 170.0, 0.13, "tri", 0.0, (0.001, 0.055, 0.10, 0.06)),
        [pl_noise[i] * pl_env[i] * 0.45 for i in range(len(pl_noise))],
    ], 0.34)
    _bank["invalid"] = _py_make([
        _py_voice(196.0, 196.0, 0.11, "tri", 0.0, (0.002, 0.05, 0.0, 0.05)),
        [v * 0.8 for v in _py_voice(208.0, 208.0, 0.11, "tri", 0.0, (0.002, 0.05, 0.0, 0.05))],
    ], 0.24)
...
def _build_bank() -> None:
    ...
    # Placing a piece: a soft wooden knock. A short low body plus a filtered noise transient —
    # the noise is what makes it read as contact rather than as a note.
    n = int(SAMPLE_RATE * 0.13)
    _bank["place"] = _make(_mix(
        _tone(np.linspace(300.0, 170.0, n, dtype=np.float32), 0.13, "tri")
        * _env(n, 0.001, 0.055, 0.10, 0.06),
        _lowpass(_noise(0.13, 23), 0.5) * _env(n, 0.001, 0.03, 0.0, 0.03) * 0.45,
    ), 0.34)

    # Rejected placement: two close low tones, deliberately dissonant and short. A buzzer would be
    # punishing on a mechanic the player will trigger by accident dozens of times a run, so this is
    # quiet and over quickly — it says "not there", not "wrong".
    n = int(SAMPLE_RATE * 0.11)
    _bank["invalid"] = _make(_mix(
        _tone(196.0, 0.11, "tri") * _env(n, 0.002, 0.05, 0.0, 0.05),
        _tone(208.0, 0.11, "tri") * _env(n, 0.002, 0.05, 0.0, 0.05) * 0.8,
    ), 0.24)
...
#
# Named for game events rather than for waveforms, so the play scene never mentions a buffer. The
# intensity of a sound is derived from the event that caused it — a bigger clear is a higher blip
# and a louder flourish — which is what keeps the audio in step with the visuals without either
# side coordinating with the other.

def pickup() -> None:
    """A piece was lifted out of the tray."""
    _play("move", 0.85)


def place() -> None:
    """A piece was set down legally."""
    _play("place")


def invalid() -> None:
    """A piece was dropped somewhere it cannot go."""
    _play("invalid", 0.8)


def line_clear(lines: int = 1, combo: int = 1) -> None:
    """Lines were cleared. Pitch rises with the combo, and the body with how many lines went.

    The pitched blips are pre-rendered a semitone apart, so choosing one is a dictionary lookup
    rather than a synth call in the middle of the frame that is also spawning 200 particles.
    """
    step = max(0, min(11, (max(1, combo) - 1) * 2 + max(0, lines - 1)))
    _play(f"eat{step}")
    if lines >= 2:
        # Layered rather than replaced: the blip is the confirmation and this is the reward, and
        # hearing both is what makes a double feel like a single plus something.
        _play("gem", 0.95)
    if lines >= 4:
        _play("level", 0.9)


def combo_up(combo: int = 2) -> None:
    """The combo counter advanced."""
    _play("combo", min(1.0, 0.6 + 0.08 * combo))


def reward() -> None:
    """Coins awarded, or a theme unlocked."""
    _play("coin")


def high_score() -> None:
    """A new best. The rising sweep, saved for exactly this."""
    _play("power")
...
def move() -> None:
    """Keyboard focus moved. Quietest thing in the game on purpose."""
    _play("move", 0.8)
